        ...
        try {
            ...
            if (res && res.status == 200) {
                ...
                if (fetchReward && fetchReward.status == 200) {
                    ...
                    if (reward.length > 0) {
                        reward.sort((a, b) => (new Date(`1997-01-01 ${a.report}:00`) > new Date(`1997-01-01 ${b.report}:00`)) ? -1 : 1).map((r) => {
                            sortReward.push(r.l_name)
                        })
                        data.map((bill, i) => {
                            bill_reward[i] = {
                                id: bill.b_id!,
                            ...
                            reward.map((r, j) => {
                                if (bill.l_id == r.l_id && bill.times == r.times) {
                                    bill.b_one_digits && (JSON.parse(bill.b_one_digits) as string[]).map((one, index) => {
                                        const ONE = one.split(":")
                                    ...
                                    bill.b_two_digits && (JSON.parse(bill.b_two_digits) as string[]).map((two, index) => {
                                        const TWO = two.split(":")
                                    ...
                                    bill.b_three_digits && (JSON.parse(bill.b_three_digits) as string[]).map((three, index) => {
                ...
                sortReward.map(s => {
                    if (!group.includes(s) && data.find(b => b.l_name == s)) group.push(s)
                })
                setLottoGroup(group)
                                                                            ...
                                                                            <>
                                                                                {
                                                                                    billReward?.map(bill_reward => (
                                                                                        (bill.l_name == g && bill_reward.id == bill.b_id) &&
                                                                                        <tr key={i} className={bill.b_status == "CANCEL" ? "bg-red-200" : bill_reward.total_win > 0 ? "bg-blue-200" : ""}>
                                                                                            <td className="border border-slate-300 font-light px-1">{moment(new Date(bill.b_created_at)).format("DD-MM-YYYY HH:mm:ss")}</td>
                                                                                            <td className="border border-slate-300 font-light">{bill.l_name!}</td>
                                                                                            <td className="border border-slate-300 font-light px-1">{moment(new Date(bill.times)).format("DD-MM-YYYY")}</td>
                                                                                            ...
                                                                                            <td className={`border border-slate-300 ${bill.b_status == "CANCEL" ? "text-red-600" : (bill_reward.total_win - parseFloat(bill_reward.bill_price.toString()) > 0) ? " text-blue-600" : " text-red-500"}`}>{bill.b_status == "CANCEL" ? "" : bill.b_status == "WAIT" ? "" : (bill_reward.total_win + bill_reward.rebate - bill_reward.bill_price) ? (parseFloat(bill_reward.total_win.toString()) + parseFloat(bill_reward.rebate.toString()) - parseFloat(bill_reward.bill_price.toString())).toLocaleString('en-us', { minimumFractionDigits: 2 }) : "0.00"}</td>
                                                                                ...
                                                                                {
                                                                                    (bill.l_name! == g && billReward.length == 0) &&
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td className="border border-slate-300 bg-gray-200 text-green-600">{
                                                                (Object.values(bills).map<number>((b: IBillMySQL, i) => {
                                                                    ...
                                                                    if (b.l_name! == g) iTotal += parseFloat(b.price) - parseFloat(b.rebate)
                                                                    return iTotal
                                                            ...
                                                            <td className="border border-slate-300 bg-gray-200">{
                                                                Object.values(bills).map<number>((b: IBillMySQL, i) => {
                                                                    let iTotal = 0;
                                                                    if (b.b_status == "CANCEL") return iTotal = 0
                                                                    if (b.l_name! == g) iTotal += parseFloat(b.rebate)
                                                                    return iTotal
                                                                }).reduce((price, cur) => price + cur, 0).toLocaleString('en-us', { minimumFractionDigits: 2 })
                                                            }</td>
                                                                ...
                                                                Object.values(bills).map<number>((b: IBillMySQL, i) => {
                                                                    ...
                                                                    if (billReward) {
                                                                        Object.values(billReward).map(bill_reward => {
                                                                            if (b.l_name! == g && bill_reward.id == b.b_id) iTotal += parseFloat(bill_reward.total_win.toString()) + parseFloat(bill_reward.rebate.toString()) - parseFloat(bill_reward.bill_price.toString())
                                                                        })
                                                                    }
                                                ...
                                                <tbody>
                                                    {
                                                        billId?.b_one_digits && (JSON.parse(billId?.b_one_digits) as string[]).map((one, i) => (
                                                            ...
                                                            <tr key={i} className={billId?.b_status != "CANCEL" && billReward.find((r) => (r.one_digit?.top?.includes(`${one.split(":")[0]}:${one.split(":")[1]}`) && r.id == billId?.b_id)) ? "bg-blue-200" : ""}>
                                                                ...
                                                                <td className={`border border-slate-300 text-center font-light`}>{billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ? (parseInt(one.split(":")[1]) * parseInt(JSON.parse(promotions[0]?.c_one_digits).top)) / 100 : (parseInt(one.split(":")[1]) * parseInt(JSON.parse(billId?.c_one_digits).top)) / 100}</td>
                                                    ...
                                                    }
                                                    {
                                                        billId?.b_one_digits && (JSON.parse(billId?.b_one_digits) as string[]).map((one, i) => (
                                                            ...
                                                            <tr key={i} className={billId?.b_status != "CANCEL" && billReward.find((r) => (r.one_digit?.bottom?.includes(`${one.split(":")[0]}:${one.split(":")[2]}`) && r.id == billId?.b_id)) ? "bg-blue-200" : ""}>
                                                                ...
                                                                <td className={`border border-slate-300 text-center font-light`}>{billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ? (parseInt(one.split(":")[1]) * parseInt(JSON.parse(promotions[0]?.c_one_digits).bottom)) / 100 : (parseInt(one.split(":")[2]) * parseInt(JSON.parse(billId?.c_one_digits).bottom)) / 100}</td>
                                                                <td className="border border-slate-300 text-center">
                                                                    <div className="flex justify-between px-1">
                                                                        <span className="font-light">{
                                                                            billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ?
                                                    ...
                                                    }
                                                    {
                                                        billId?.b_two_digits && (JSON.parse(billId?.b_two_digits) as string[]).map((two, i) => (
                                                            ...
                                                            <tr key={i} className={billId?.b_status != "CANCEL" && billReward.find((r) => (r.two_digit?.top?.includes(`${two.split(":")[0]}:${two.split(":")[1]}`)) && r.id == billId?.b_id) ? "bg-blue-200" : ""}>
                                                                ...
                                                                <td className={`border border-slate-300 text-center font-light`}>{billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ? (parseInt(two.split(":")[1]) * parseInt(JSON.parse(promotions[0]?.c_two_digits).top)) / 100 : (parseInt(two.split(":")[1]) * parseInt(JSON.parse(billId?.c_two_digits).top)) / 100}</td>
                                                                <td className="border border-slate-300 text-center">
                                                                    <div className="flex justify-between px-1">
                                                                        <span className="font-light">{
                                                                            billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ?
                                                    ...
                                                    }
                                                    {
                                                        billId?.b_two_digits && (JSON.parse(billId?.b_two_digits) as string[]).map((two, i) => (
                                                            ...
                                                            <tr key={i} className={billId?.b_status != "CANCEL" && billReward.find((r) => (r.two_digit?.bottom?.includes(`${two.split(":")[0]}:${two.split(":")[2]}`) && r.id == billId?.b_id)) ? "bg-blue-200" : ""}>
                                                                ...
                                                                <td className={`border border-slate-300 text-center font-light`}>{billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ? (parseInt(two.split(":")[1]) * parseInt(JSON.parse(promotions[0]?.c_two_digits).bottom)) / 100 : (parseInt(two.split(":")[2]) * parseInt(JSON.parse(billId?.c_two_digits).bottom)) / 100}</td>
                                                                <td className="border border-slate-300 text-center">
                                                                    <div className="flex justify-between px-1">
                                                                        <span className="font-light">{
                                                                            billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ?
                                                    ...
                                                    }
                                                    {
                                                        billId?.b_three_digits && (JSON.parse(billId?.b_three_digits) as string[]).map((three, i) => (
                                                            ...
                                                            <tr key={i} className={billId?.b_status != "CANCEL" && billReward.find((r) => (r.three_digit?.top?.includes(`${three.split(":")[0]}:${three.split(":")[1]}`) && r.id == billId?.b_id)) ? "bg-blue-200" : ""}>
                                                                ...
                                                                <td className={`border border-slate-300 text-center font-light`}>{billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ? (parseInt(three.split(":")[1]) * parseInt(JSON.parse(promotions[0]?.c_three_digits).top)) / 100 : (parseInt(three.split(":")[1]) * parseInt(JSON.parse(billId?.c_three_digits).top)) / 100}</td>
                                                                <td className="border border-slate-300 text-center">
                                                                    <div className="flex justify-between px-1">
                                                                        <span className="font-light">{
                                                                            billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ?
                                                    ...
                                                    }
                                                    {
                                                        billId?.b_three_digits && (JSON.parse(billId?.b_three_digits) as string[]).map((three, i) => (
                                                            ...
                                                            <tr key={i} className={billId?.b_status != "CANCEL" && billReward.find((r) => (r.three_digit?.toad?.includes(`${three.split(":")[0]}:${three.split(":")[2]}`) && r.id == billId?.b_id)) ? "bg-blue-200" : ""}>
                                                                ...
                                                                <td className={`border border-slate-300 text-center font-light`}>{billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ? (parseInt(three.split(":")[1]) * parseInt(JSON.parse(promotions[0]?.c_three_digits).toad)) / 100 : (parseInt(three.split(":")[2]) * parseInt(JSON.parse(billId?.c_three_digits).toad)) / 100}</td>
                                                                <td className="border border-slate-300 text-center">
                                                                    <div className="flex justify-between px-1">
                                                                        <span className="font-light">{
                                                                            billId?.promotion == "USED" && promotions[0] && JSON.parse(promotions[0]?.p_promotion.toString()).includes(day[new Date(billId?.times).getDay()]) ?
                                                                                digitSemi && JSON.parse(digitSemi!.three_digits as string).toad.includes(three.split(":")[0]) ?
                                                                                    (JSON.parse(promotions[0]?.rt_three_digits).toad / 2).toFixed(2) :
                                                                                    JSON.parse(promotions[0]?.rt_three_digits).toad :

                                                                                digitSemi && JSON.parse(digitSemi!.three_digits as string).toad.includes(three.split(":")[0]) ?
                                                                                    (JSON.parse(billId?.rt_three_digits).toad / 2).toFixed(2) :
